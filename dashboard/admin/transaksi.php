                ...
                <div class="col-md-4">
                    <form action="fungsi/prosesTransaksi.php" method="POST">
                            ...
                            <select class="form-control" onchange='location=this.value' required>
                                ...
                                <?php if (isset($_GET['meja'])) : ?>
                                    <?php foreach ($kursi as $kursi) : ?>
                                        <option value="index.php?meja=<?= $kursi['meja_id'] ?>" <?= $kursi['meja_id'] == $_GET['meja'] ? 'selected' : '' ?>><?= $kursi['meja_id'] ?></option>
                                    <?php endforeach; ?>

                                <?php else : ?>
                                    <?php foreach ($kursi as $kursi) : ?>
                                        <option value="index.php?meja=<?= $kursi['meja_id'] ?>"><?= $kursi['meja_id'] ?></option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control totbayar" readonly required value="<?= $toto ?>" name="total_bayar" placeholder="Total Bayar">
                        </div>
                        <!-- <div class="form-group">
                            <label for="">Keterangan</label>
                            <input type="text" class="form-control keterangan" min="0" max="100" name="keterangan" placeholder="Keterangan ">
                        </div> -->
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-6">
